...
/*	printf("%s\n",(char*) message->payload);*/
	...
	strftime(buffer, sizeof(buffer), "/tmp/to_import%d%m%Y_%H%M%S.json", timenow);
	...
	fclose(fp);	
	system("ls -1 /tmp/*.json | sed 's/.json$//' | while read col; do mongoimport -d IoT-Dashboard -c readings <$col.json > /dev/null ;done");
	system("rm /tmp/*.json");
}
